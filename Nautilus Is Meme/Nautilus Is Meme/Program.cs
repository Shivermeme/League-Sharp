...
namespace Nautilus_Is_Meme
{
    class Program
    {
        ...
        static void Game_OnGameLoad(EventArgs args)
        {
            ...
            Game.PrintChat("Nautilus the Memer Lord");
        ...
        private static void CastQ()
        {
            ...
            if (hitchance == HitChance.High || hitchance == HitChance.Immobile || hitchance == HitChance.Dashing ||
                hitchance == HitChance.VeryHigh && !target.IsStunned && !target.IsImmovable)
            {
                Q.Cast(target);
            }
        }
        ...
        {
            if (W.IsReady() && ObjectManager.Get<Obj_AI_Hero>().Where(x => x.IsEnemy && x.Distance(Player) <= 1000).Count() > 0)
            {
                W.Cast();
            }
        }
        ...
        {
            if (R.IsReady() && target.Distance(Player) <= R.Range && !target.IsStunned && !target.IsImmovable)
            {
